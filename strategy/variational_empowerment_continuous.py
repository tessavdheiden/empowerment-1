...
class VariationalEmpowermentContinuous(EmpowermentStrategy):
    ...
    def __init__(self, n_s, n_a, n_step):
        self.input_dim = n_s
        self.n_step = n_step
        self.source = Source(2, 2**n_step).to(device)
        self.planner = Planner(2, 2**n_step).to(device)
    ...
    def train_batch(self, world, T2, n_step, n_samples=int(5e5)):
        """
        ...
        c_all = self._index_to_cell(s_all, world.dims[0], world.dims[1])
        start = time.time()
