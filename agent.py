...
class EmpMaxAgent:
    ...
    def setE(self, world, T, n_step, strategy):
        self.E = strategy.compute(world=world, T=T, n_step=n_step).reshape(-1)
    ...
    def estimateE(self, state):
        ...
        if self.n_a ** self.n_step < 5000:
            nstep_samples = np.array(list(itertools.product(range(self.n_a), repeat=self.n_step)))
        else:
            nstep_samples = np.random.randint(0, self.n_a, [self.n_samples, self.n_step])
        # fold over each nstep actions, get unique end states
        tmap = lambda s, a: np.argmax(self.T[:, a, s])
        seen = set()
