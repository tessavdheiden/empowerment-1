...
def example_8():
    ...
    w.plot(fig, ax[1, 2], colorMap=a1.brain.value_map.reshape(*w.dims), traj=traj)
...
def example_9():
    ...
    for t in range(steps):
        ...
        if t % 500 == 0:
            ...
            print(f'low V ={a0.value_map[idx[0]]:.2f} high V ={a0.value_map[idx[-1]]:.2f}')
            # second agent
            idx = np.argsort(a1.E)
            for j, agent in enumerate(w.agents):
                agent.s = w._index_to_location(idx[0], j)
            ...
            # some statistics
            ax[2, 0].pcolor(a0.action_map.reshape(w.dims[0], -1))
            ax[2, 0].set_title(f"agent 0 value map")

            ax[2, 1].pcolor(a1.value_map.reshape(w.dims[0], -1))
